...
export default function ChatPage() {
  ...
  return (
    <div className="h-[calc(100vh-4rem)] flex flex-col">
      <div className="flex items-center gap-4 px-6 py-4 border-b border-[var(--color-border-default)] bg-[var(--color-bg-secondary)] flex-shrink-0">
        <h1 className="font-heading font-bold text-lg">Чат</h1>
        <div className="flex gap-1 p-1 rounded-[12px] bg-[var(--color-bg-primary)] border border-[var(--color-border-default)]">
          {visibleTabs.map((t) => (
            <button
              key={t.id}
              onClick={() => setTab(t.id)}
              className={clsx(
                'flex items-center gap-1.5 px-4 py-2 rounded-[10px] text-sm font-medium transition-all',
                tab === t.id
